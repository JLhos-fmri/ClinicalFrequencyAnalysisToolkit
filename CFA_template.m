...
Htemp.tempsel12 = uicontrol('parent',Htemp.tempbg,...
    ...
    'string','Other',...
    'val',0);
Htemp.tempsel12_1 = uicontrol('parent',Htemp.tempbg,...
    'unit','norm',...
    'pos',[3/4,1/9,1/4,1/9],...
    'style','edit',...
    'string','Template dir',...
    'enable','off');
Htemp.tempsel12_2 = uicontrol('parent',Htemp.tempbg,...
    'unit','norm',...
    'pos',[3/4,0/9,1/4,1/9],...
    'style','edit',...
    'string','ROIname dir',...
...
set(Htemp.tempsel12,'callback',{@SelOtherTemp,Htemp});
...
Htemp = varargin{3};
VALs = get(Htemp.tempsel12,'val');
if VALs==1
    [file,pth] = uigetfile({'*.nii';'*.nii.gz';'*.hdr';'*.img'},'Select ROI template');
    PGs = fullfile(pth,file);
    set(Htemp.tempsel12_1,'enable','on');
    set(Htemp.tempsel12_1,'string',PGs);
    ...
    set(Htemp.tempsel12_2,'enable','on');
    set(Htemp.tempsel12_2,'string',PGs2);
else    
    set(Htemp.tempsel12_1,'enable','off');
    set(Htemp.tempsel12_2,'enable','off');
    set(Htemp.tempsel12_1,'string','');
    set(Htemp.tempsel12_2,'string','');    
end
end
...
Template.Other = get(Htemp.tempsel12,'val');
Template.OtherROI = get(Htemp.tempsel12_1,'string');
Template.OtherROIname = get(Htemp.tempsel12_2,'string');
%%
...
Template.OtherROIname = get(Htemp.tempsel12_2,'string');
%%
ParaSet.Heat.V = get(Htemp.heatpara_heatnumSelV,'val');
ParaSet.Heat.R = get(Htemp.heatpara_heatnumSelROI,'val');
ParaSet.Heat.Rper = get(Htemp.heatpara_heatnumSelROIedit,'string');
ParaSet.Heat.StChisquare = get(Htemp.heatpara_heatnumStatC,'val');
ParaSet.Heat.StPermutation = get(Htemp.heatpara_heatnumStatP,'val');

ParaSet.CentNum.R = get(Htemp.heatpara_CentSelROI,'val');
ParaSet.CentNum.StChisquare = get(Htemp.heatpara_CentStatC,'val');
ParaSet.CentNum.StPermutation = get(Htemp.heatpara_CentStatP,'val');

ParaSet.AffVolume.R = get(Htemp.heatpara_AffVolSelROI,'val');
ParaSet.AffVolume.StPermutation = get(Htemp.heatpara_AffVolStatP,'val');

ParaSet.WeiCent.R = get(Htemp.heatpara_WeightedCentSelV,'val');
ParaSet.WeiCent.StPermutation = get(Htemp.heatpara_AffVolStatP,'val');

ParaSet.WeiCent.V = get(Htemp.heatpara_WeightedCentSelV,'val');
ParaSet.WeiCent.R = get(Htemp.heatpara_WeightedCentSelROI,'val');
% ParaSet.WeiCent.Rper = get(Htemp.heatpara_WeightedCentSelROIedit,'string');
ParaSet.WeiCent.StPermutation = get(Htemp.heatpara_WeightedCentStatP,'val');

% ParaSet.Other = get(Htemp.heatpara_Otherparapb,'string')
%%
save('Setup_TempAndPara.mat','ParaSet','Template');
if isempty(dir(Htemp.ParaSavedir))
    mkdir(Htemp.ParaSavedir);
end
save([Htemp.ParaSavedir,filesep,'Setup_TempAndPara.mat'],'ParaSet','Template')
close(Htemp.fig);
movegui(H.fig,'center')
end
